...
Unlike conventional operating systems retrofitted with AI wrappers, ActonOS places the **Agent Engine**, **Sandboxed Tool Runtime**, **Hardware-Bound Security Vault**, and **Hybrid Memory System** directly into the core system layer.

```mermaid
graph TB
    subgraph "Layer 1 - Connectivity & Access"
        TSNET["Tailscale tsnet<br/>E2E Encrypted Mesh VPN"]
        WEBUI["Web UI SPA<br/>React 19 + Tailwind v4 + Soft Meadow"]
        CHANNELS["Omnichannel Adapters<br/>Telegram · Discord · Slack · Webhook"]
    ...
    subgraph "Layer 3 - Dynamic Tooling Hub"
        MCP["MCP Host Engine<br/>stdio / SSE Transports"]
        WASM["WASM Runtime<br/>wazero Pure Go"]
...
### 3. Dynamic Hot-Swappable Tooling
ActonOS supports three pluggable tool protocols with zero daemon restarts:
- **Model Context Protocol (MCP)**: Native stdio and HTTP/SSE JSON-RPC client for external tool servers.
- **Pure Go WebAssembly (WASM)**: Ultra-fast, sandboxed plugins executed with `wazero`.
