...
graph LR
    ...
    F --> G["Lưu Manifest Agent<br/>agent_manifests.json"]
```
...
2. Lưu các tệp kết quả tạm thời trong thư mục `/workspace/scratch/`.
...
- **Ngưỡng Phê duyệt**:
  - `Luôn hỏi`: Bắt buộc con người phê duyệt mọi thao tác ghi/sửa.
  - `Chỉ thao tác nguy cơ cao`: Yêu cầu phê duyệt khi sửa file, chạy lệnh shell hoặc gọi API bên ngoài.
  - `Tự trị hoàn toàn`: Tự động thực thi không cần ngắt quãng.
