...
## 9. Tools, MCP, and Skills catalog

### `GET /api/tools`
Lists registered tools (native, MCP, WASM plugins, and folder skills). The **Native Tools** page hides the `skill` category; those appear on **Skills**.

### `POST /api/tools/mcp` | `DELETE /api/tools/mcp/{serverID}`
Register or disconnect an MCP server (`stdio`, Streamable HTTP, or `sse`). High-risk operations return a `202 Accepted` pending approval request.

### `GET /api/tools/hub/catalog`
Fetches the community **Skills** catalog shown on **Extensions → Skills → Community Hub**.

### `POST /api/tools/hub/install` | `POST /api/tools/hub/uninstall`
Installs or uninstalls a community skill package.
