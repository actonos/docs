...
graph LR
    ...
    F --> G["Saved Agent Manifest<br/>agent_manifests.json"]
```
...
2. Store temporary diagnostic output in `/workspace/scratch/`.
...
- **Approval Threshold**:
  - `Always Ask`: Requires human approval for all mutations.
  - `High-Risk Only`: Requires approval for file writes, shell execution, and SaaS mutations.
  - `Autonomous`: Executes all authorized tools automatically without interruption.
