...
Chat attachments are for **this conversation**. To keep a document for later, ask the agent to save it to the [Workspace](workspace) (for example: “Save this as `brief.pdf` in the workspace”). Workspace files keep their original names, so you can later say “Open `brief.pdf`” or “Send `brief.pdf` to Telegram.”

### Send a workspace file to a chat app

If Telegram, Discord, Slack, WhatsApp, or Zalo is installed, you can ask in chat:
...
When an agent invokes external tools (e.g., executing a Python script or browsing a website), ActonOS renders an interactive **Tool Execution Card** directly in the message timeline:
