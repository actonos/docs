...
## 6. Channel plugins send the files

When an agent sends a document to Telegram, Discord, Slack, WhatsApp, or Zalo, ActonOS **only forwards the file** to the installed channel plugin. The plugin is responsible for uploading it to that chat app.

That split matters in two ways:

1. **You install the chat app as a plugin.** There is no hardcoded Telegram or Zalo uploader inside the core OS.
2. **You should re-install channel plugins after an ActonOS update** so they match the current file-delivery format. Older plugins may report success while the file never appears (especially on Zalo), or a downloaded PDF may open with broken fonts.

To refresh a plugin: **Uninstall**, then **Upload** the latest `.actonpkg` from the official channel packages.

For how sending looks from the user’s side, see [Sending files to chat](channels#sending-files-to-chat). Plugin authors: see [WASM plugin development — sending files](../developer-reference/wasm-plugin-development#sending-files).
