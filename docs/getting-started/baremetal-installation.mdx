...
```mermaid
flowchart LR
    A["Download ISO<br/>ActonOS-v1.iso"] --> B["Flash to USB<br/>Rufus / BalenaEtcher / dd"]
...
4. Click **Select** and choose `ActonOS-v1-amd64.iso`.
...
2. Click **Flash from file** and select `ActonOS-v1-amd64.iso`.
...
sudo dd if=ActonOS-v1-amd64.iso of=/dev/sdX bs=4M status=progress conv=fsync
```
