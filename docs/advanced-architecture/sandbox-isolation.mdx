...
graph TD
    ...
    subgraph "OS-Level Sandbox (bwrap + Cgroups v2)"
        D["Mount Namespaces:<br/>• /usr, /lib, /bin -> Read-Only<br/>• /proc -> Isolated Procfs<br/>• scratchpad + user-workspace (original names) -> Scoped Read-Write"]
...
- **Workspace Access**: User documents are available by **original filename** (for example `report.pdf`) through the named view `user-workspace/` and the environment variable `ACTONOS_USER_WORKSPACE`. The agent’s private scratchpad is a separate writable folder. Host config and Vault paths stay hidden.
