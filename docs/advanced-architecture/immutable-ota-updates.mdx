...
sequenceDiagram
    ...
    Cloud->>OTA: Notify New Version (v0.2.0)
    OTA->>FS: Download binary to /data/releases/v0.2.0/actond
    OTA->>OTA: Verify SHA-256 Checksum + GPG Signature
    OTA->>FS: Atomic Symlink Swap: /data/bin/actond -> releases/v0.2.0/actond
    ...
    loop Every 5s for 30s
        ...
        alt HTTP 200 OK & Status Healthy
            WD->>FS: Mark v0.2.0 as Stable Release ✅
...
```
/data/bin/actond -> /data/releases/v0.1.0/actond
```

When an update is applied:
1. The new binary is downloaded and verified in isolation under `/data/releases/v0.2.0/`.
